        ...
        return http
                ...
                .authorizeHttpRequests(req ->
                        req.requestMatchers("/lifepill/v1/auth/**",
                                        ...
                                        "/lifepill/v1/search/**",
                                        // Inter-service communication endpoints (called by other microservices)
                                        "/lifepill/v1/sales/**"
                                ).permitAll()
                                .requestMatchers("/lifepill/v1/branch-summary/**")
                                .hasRole(OWNER.name())
                                .requestMatchers("/lifepill/v1/admin/**")
                                .hasRole(OWNER.name())
                                .requestMatchers("/lifepill/v1/cashierNew/**")
                                .hasAnyRole(CASHIER.name(), MANAGER.name(), OWNER.name())
                                .requestMatchers("/lifepill/v1/branch/**")
                                .hasRole(OWNER.name())
                                .requestMatchers("/lifepill/v1/employers/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name())
                                .requestMatchers("/lifepill/v1/cashier/**")
                                .hasAnyRole(CASHIER.name(), MANAGER.name(), OWNER.name())
                                .requestMatchers("/lifepill/v1/owner/**")
                                .hasRole(OWNER.name())
                                .requestMatchers("/lifepill/v1/branch-manager/**")
                                .hasAnyRole(MANAGER.name(), OWNER.name())
                                .requestMatchers("/lifepill/v1/item-Category/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name(), CASHIER.name())
                                .requestMatchers("/lifepill/v1/item/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name(), CASHIER.name())
                                .requestMatchers("/lifepill/v1/order/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name(), CASHIER.name())
                                .requestMatchers("/lifepill/v1/supplierCompanies/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name(), CASHIER.name())
                                .requestMatchers("/lifepill/v1/supplier/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name(), CASHIER.name())
                                // Proxy endpoints - require authentication, roles checked in controllers
                                .requestMatchers("/lifepill/v1/proxy/branch/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name(), CASHIER.name())
                                .requestMatchers("/lifepill/v1/proxy/inventory/**")
                                .hasAnyRole(OWNER.name(), MANAGER.name(), CASHIER.name())
