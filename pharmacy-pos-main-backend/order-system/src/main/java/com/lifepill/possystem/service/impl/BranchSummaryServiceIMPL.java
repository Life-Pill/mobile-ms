...
import com.lifepill.possystem.client.BranchServiceClient;
...
public class BranchSummaryServiceIMPL implements BranchSummaryService {
    ...
    public AllPharmacySummaryResponseDTO getAllPharmacySummary() {
        ...
        // Fetch total employees via Identity Service
        long totalEmployees = 0;
        try {
            ResponseEntity<MicroserviceApiResponse<Long>> employeeResponse = 
                identityServiceClient.countEmployeesByBranch(null);
            if (employeeResponse != null && employeeResponse.getBody() != null && 
                employeeResponse.getBody().isSuccess() && employeeResponse.getBody().getData() != null) {
                totalEmployees = employeeResponse.getBody().getData();
            }
        } catch (Exception e) {
            logger.warn("Could not fetch employee count from Identity Service");
        }

        // Fetch total branches via Branch Service
        long totalBranches = 0;
        try {
            var branchResponse = branchServiceClient.getAllBranches();
            if (branchResponse != null && branchResponse.getBody() != null && 
                branchResponse.getBody().isSuccess() && branchResponse.getBody().getData() != null) {
                totalBranches = branchResponse.getBody().getData().size();
            }
        } catch (Exception e) {
            logger.warn("Could not fetch branch count from Branch Service");
        }
    ...
    /**
     * Retrieves the manager's first name for the given branch ID via Identity Service.
     *
    ...
    private String getManagerForBranch(Long branchId) {
        try {
            ResponseEntity<MicroserviceApiResponse<MicroserviceEmployeeDTO>> response = 
                identityServiceClient.getManagerByBranch(branchId);
            if (response != null && response.getBody() != null && 
                response.getBody().isSuccess() && response.getBody().getData() != null) {
                MicroserviceEmployeeDTO manager = response.getBody().getData();
                return manager.getEmployerFirstName();
            }
        } catch (Exception e) {
            logger.warn("Could not fetch manager from Identity Service for branchId: {}", branchId);
        }
        return "No Manager Assigned";
    }

    /**
     * Retrieves branch details for the given branch ID via Branch Service.
     *
     * @param branchId The ID of the branch.
    ...
    private BranchDTO getBranchDetails(Long branchId) {
        try {
            var responseEntity = branchServiceClient.getBranchById(branchId);
            if (responseEntity != null && responseEntity.getBody() != null && 
                responseEntity.getBody().isSuccess() && responseEntity.getBody().getData() != null) {
                MicroserviceBranchDTO branchData = responseEntity.getBody().getData();
                return modelMapper.map(branchData, BranchDTO.class);
            }
            throw new NotFoundException("No Branch found for that id");
        } catch (Exception e) {
            logger.warn("Could not fetch branch details for branchId: {}", branchId);
            // Return a minimal BranchDTO with just the ID
            BranchDTO branchDTO = new BranchDTO();
            branchDTO.setBranchId(branchId);
            branchDTO.setBranchName("Branch " + branchId);
            return branchDTO;
        }
    }
